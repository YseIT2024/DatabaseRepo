CREATE PROCEDURE [account].[spGetPickUpDropPaymentDetails] --8605
(
    @TransactionID int,
	@DrawerID int=0,
...
BEGIN
	...
	tm.TransactionMode,
	c.currencycode,
	--CONCAT(att.TransactionType, ' ', c.CurrencySymbol, ' ', FORMAT(atm.ActualAmount, 'N2'), ' PickUpAndDrop Payment') AS Description,
	...
	INNER JOIN account.[Transaction] atz ON atz.TransactionID = atm.TransactionID
    WHERE 
	...
	tm.TransactionMode,
	c.currencycode,
	--CONCAT(att.TransactionType, ' ', c.CurrencySymbol, ' ', FORMAT(atm.Amount, 'N2'),'PickUpAndDrop Payment') AS Description,
	atz.Remarks as Description,
