...
CREATE  PROCEDURE [general].[Load_DimDate]
@StartDate DATE = '2019-11-01',
@EndDate DATE = '2045-12-31'
...
BEGIN
	...
	TRUNCATE TABLE [general].[DimDate];
...
--                               FROM int.Lineage
--                               WHERE [TableName] = N'[general].[DimDate]'
--                               AND [FinishLoad] IS NULL
--                               ORDER BY [LineageKey] DESC);

IF NOT EXISTS (SELECT 1 FROM [general].[DimDate] WHERE [Date Key] = 0)
INSERT INTO [general].[DimDate]
           ([Date Key]
           ,[Date]
           ,[Day]
           ,[Day Suffix]
...
SELECT * FROM [general].[DimDate]


    
	-- Update the lineage table for the most current DimDate load with the finish date and 
 ...
 --   WHERE [LineageKey] = @LineageKey;
	...
	---- Update the LoadDates table with the most current load date for DimDate
