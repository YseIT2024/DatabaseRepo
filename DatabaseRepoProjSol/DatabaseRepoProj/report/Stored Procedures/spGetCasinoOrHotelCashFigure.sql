...
CREATE PROCEDURE [report].[spGetCasinoOrHotelCashFigure] --1,1,1
(
...
BEGIN	
	...
	d.DenominationValue [Denomination], ds.DenomQuantity [Quantity], ds.DenomTotalValue [DenomTotal], c.CurrencySymbol
	...
	EXEC [report].[spGetCasinoOrHotelCashFigureSummary] @DenominationValueTypeID, @DrawerID, @AccountingDateId

	SELECT FORMAT(GETDATE(), 'dd-MMM-yyyy hh:mm tt') [Date], FORMAT(@AccountingDate,'dd-MMM-yyyy') [AccountingDate]

	-----Insert in Log Table -----------------
	EXEC [app].[spInsertActivityLog] 4, @LocationID , 'Casino Or Hotel Cashfigure', @UserID
END