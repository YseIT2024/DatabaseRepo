...
-- =============================================
CREATE PROCEDURE [report].[spGetDenominationValuesTotalsEURO] --'2019-12-02',1
(
	@AccountingDate DATE,
	@DrawerID INT
)
...
BEGIN
	...
	WHERE(dvs.DrawerID = @DrawerID) AND (dvs.AccountingDateID = @TodayAccountingDateID) AND 
	 rc.IsActive =1 AND    (rc.AccountingDateId = @TodayAccountingDateID ) AND rc.DrawerID = @DrawerID AND dt.DenominationTypeID =3
	...
	WHERE     (dvs.DrawerID = @DrawerID) AND (dvs.AccountingDateID = @PreviousAccountingDateID) AND 
	 rc.IsActive =1 AND (rc.AccountingDateId = @PreviousAccountingDateID ) and rc.DrawerID = @DrawerID AND dt.DenominationTypeID =3
	ORDER BY d.DenominationTypeID                      
...
/************************************************************/
	...
	UPDATE @OpeningUSDTable SET DenominationTypeCode = 'EURO CASH',MovementValueUSD  = @MovementValueUSD , MovementQuantityUSD = @MovementUSDQuantity 
