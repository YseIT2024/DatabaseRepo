...
-- =============================================
CREATE PROCEDURE [report].[spGetDenominationValuesTotalsUSD] ---'2019-12-02',1
(
	@AccountingDate DATE,
	@DrawerID INT
)
...
BEGIN
	...
	WHERE(dvs.DrawerID = @DrawerID) AND (dvs.AccountingDateID = @TodayAccountingDateID) AND 
	 rc.IsActive =1 AND    (rc.AccountingDateId = @TodayAccountingDateID ) AND rc.DrawerID = @DrawerID AND dt.DenominationTypeID = 1
	...
	rc.IsActive =1 AND (rc.AccountingDateId = @PreviousAccountingDateID ) and rc.DrawerID = @DrawerID AND dt.DenominationTypeID = 1 
	ORDER BY d.DenominationTypeID                     
...
/************************************************************/
	...
	UPDATE @OpeningUSDTable SET DenominationTypeCode = 'USD CASH',MovementValueUSD  = @MovementValueUSD , MovementQuantityUSD = @MovementUSDQuantity 
