...
CREATE PROCEDURE [fund].[spGetFundTransferDetails] 
(
	@DrawerID int	
)
AS
BEGIN
	...
	WHERE ff.DrawerID = @DrawerID AND ff.AccountingDateID = (SELECT AccountingDateID FROM account.AccountingDates WHERE DrawerID = @DrawerID AND IsActive = 1)
