...
CREATE Proc [room].[spGetRoomRates] --1
(
	@LocationID int
)
AS
BEGIN
	...
	WHERE r.IsActive = 1 AND r.LocationID = @LocationID AND (fd.Date IS NULL OR fd.Date >= CONVERT(DATE,GETDATE()))	AND r.DurationID <> 3
