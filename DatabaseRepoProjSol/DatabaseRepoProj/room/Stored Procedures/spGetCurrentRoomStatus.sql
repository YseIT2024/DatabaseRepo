...
CREATE PROCEDURE [room].[spGetCurrentRoomStatus] --1
(
	@LocationID INT,
...
BEGIN
	...
	IF (@Status = 0)-----All Rooms based on location---------
		BEGIN
			...
			,R.RoomStatusID as [PrimaryStatusID]
			,pr.RoomStatus as [PrimaryStatus]			
			...
			,R.RoomStatusID as [SecondaryStatusID]
			,pr.RoomStatus as [SecondaryStatus]
			--,ISNULL(roomStatus.ReservationID,0) ReservationID
			--,1	ReservationID 			
			,CASE WHEN r.RoomStatusID =5 THEN
				isnull((SELECT top 1 ReservationID  FROM [Products].[RoomLogs] where RoomID=r.roomid and RoomStatusID in (5)),0) --and @CurrentDate between FromDateID and ToDateId) ,0)
				--isnull(r.ResId,0)
			else 0
			end as ReservationID
		--,CASE WHEN vwr.ReservationID IS NULL THEN '' ELSE vwr.FullName END as [Name]	
			,CASE WHEN r.RoomStatusID =5 THEN 
				(select [guest].[fnGetGuestFullName] (rr.GuestId) from reservation.Reservation rr 
					where rr.reservationid=(SELECT top 1 ReservationID  FROM [Products].[RoomLogs] where RoomID=r.roomid and RoomStatusID =5) )
			Else ''
			End as [Name]
		--,'abc'
		--	,CASE WHEN primaryStatus.RoomStatusID = 5 THEN FORMAT(vwr.ExpectedCheckIn,'dd-MMM-yyyy') ELSE '' END [ExpectedCheckIn]
			,CASE WHEN r.RoomStatusID = 5 THEN 
				(select FORMAT(rr.ExpectedCheckIn,'dd-MMM-yyyy') from reservation.Reservation rr where rr.reservationid=(SELECT top 1 ReservationID  FROM [Products].[RoomLogs] where RoomID=r.roomid and RoomStatusID =5) ) ELSE '' END [ExpectedCheckIn]
			--,CASE WHEN primaryStatus.RoomStatusID = 5 THEN FORMAT(vwr.ActualCheckIn,'dd-MMM-yyyy') ELSE '' END [CheckedIn]
			,CASE WHEN r.RoomStatusID = 5 THEN --1 ELSE '' END CheckedIn
			--	--(select FORMAT(rr.ActualCheckIn,'dd-MMM-yyyy') from reservation.Reservation rr where rr.reservationid=r.ResId ) ELSE '' END CheckedIn
				(select FORMAT(rr.ActualCheckIn,'dd-MMM-yyyy') from reservation.Reservation rr where rr.reservationid=(SELECT top 1 ReservationID  FROM [Products].[RoomLogs] where RoomID=r.roomid and RoomStatusID =5) )  ELSE '' END [CheckedIn]
			...
			,CASE WHEN r.RoomStatusID = 5 THEN 
			----(select FORMAT(rr.ExpectedCheckOut,'dd-MMM-yyyy') from reservation.Reservation rr where rr.reservationid=r.ResId ) ELSE '' END [ExpectedCheckOut]
				(select FORMAT(rr.ExpectedCheckOut,'dd-MMM-yyyy') from reservation.Reservation rr where rr.reservationid=(SELECT top 1 ReservationID  FROM [Products].[RoomLogs] where RoomID=r.roomid and RoomStatusID =5 )) ELSE '' END [ExpectedCheckOut]
			--,ISNULL(roomStatus.RSHistoryID,0) [RoomStatusHistoryID]
			--,CASE WHEN r.RoomStatusID = 5 THEN  
				,isnull((SELECT top 1 rshistoryid  FROM [Products].[RoomLogs] where RoomID=r.roomid and RoomStatusID =5),'')
			--else ''
			--End [RoomStatusHistoryID]
			--,''
			...
			WHERE r.LocationID = 1 AND r.IsActive = 1
			...
			[Name], ExpectedCheckIn, CheckedIn, ExpectedCheckOut, RoomStatusHistoryID FROM @Temp
			ORDER BY RoomNo
		END
