...
CREATE PROCEDURE [reservation].[GetProductDetails_New] --45,'12/18/2023','12/21/2023',0,0,0,0
(	
	@SubCategoryID int,
...
BEGIN
		...
		--Begin Commented by Arabinda on 2023-09-27 to get the counts from subcategory tables------------------
			...
			--AND (PRP.FromDate>= @ExpectedCheckInDate And PRP.FromDate<=@ExpectedCheckOutDate)   -- Added BY Somnath
			WHERE SubCategoryID=@SubCategoryID and GC.CompanyID = @CompanyID	and it.IsActive=1 and GC.IsActive=1
