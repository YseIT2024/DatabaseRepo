...
CREATE PROCEDURE [reservation].[GetProductRateDetails_New_sp_BK2512223]
(	
...
BEGIN
		...
		--Declare 
		--@dtProducts [reservation].[dtProductsRates],
		--@ExpectedCheckInDate datetime='1/08/2023',@ExpectedCheckOutDate datetime='1/10/2023',@NoOfAdults int=4,
	 --   @NoOfChilds int=1,@NoOfRooms int=1
		--insert into @dtProducts values(151,'Delux Room',1)
		--select * from @dtProducts
...
(
	...
	BeforeTax decimal(18,6)
			...
			BEGIN
				...
				,@NoOfRooms RoomCount,
				(case when @IntExAdult>0 OR @IntExChild>0 OR @IntExChildSr>0 then PRP.SalePrice + (@IntExAdult*ISNULL(prp.AddPax,0)) + (@IntExChild*ISNULL(prp.AddChild,0)) + (@IntExChildSr*ISNULL(prp.AddChildSr,0)) else PRP.SalePrice END) - (ISNULL((case when @IntExAdult>0 OR @IntExChild>0 OR @IntExChildSr>0 then PRP.SalePrice + (@IntExAdult*ISNULL(prp.AddPax,0)) + (@IntExChild*ISNULL(prp.AddChild,0)) + (@IntExChildSr*ISNULL(prp.AddChildSr,0)) else PRP.SalePrice END) / ( 1+ sum(GT.TaxRate)),0)) as BeforeTax
