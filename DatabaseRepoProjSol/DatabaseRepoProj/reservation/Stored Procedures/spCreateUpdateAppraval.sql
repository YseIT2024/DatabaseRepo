...
CREATE PROCEDURE [reservation].[spCreateUpdateAppraval]
(
					...
					BEGIN
						IF (@ProcesstypeId=1)
							BEGIN
								...
								IF NOT EXISTS(SELECT * FROM [reservation].[ApprovalLog] WHERE ProcesstypeId in (2,5,6,7) AND RefrenceNo=@RefrenceNo AND ApprovalStatus in(0,2,3) AND ApprovalLogId=(SELECT top(1) ApprovalLogId FROM [reservation].[ApprovalLog] WHERE ProcesstypeId in (2,5,6,7) AND RefrenceNo=@RefrenceNo order by ApprovalLogId desc))
									BEGIN
						...
						IF (@ProcesstypeId=2)
								...
								IF NOT EXISTS(SELECT * FROM [reservation].[ApprovalLog] WHERE ProcesstypeId in (1,5,6,7) AND RefrenceNo=@RefrenceNo AND ApprovalStatus in(0,2,3) AND ApprovalLogId=(SELECT top(1) ApprovalLogId FROM [reservation].[ApprovalLog] WHERE ProcesstypeId in (1,5,6,7) AND RefrenceNo=@RefrenceNo order by ApprovalLogId desc))
									BEGIN
										UPDATE reservation.Reservation SET AuthorizedFlag=0 WHERE ReservationID=@RefrenceNo
									END
								END
						...
						IF (@ProcesstypeId=4)
							BEGIN
								--SET @ApprovalDescripion ='Room/Meal Plan Tariff Changed'
								 ...
								 -- Price Details Moved from Temp Table To  RoomPrice Table
								 exec [reservation].[spTransferDataTempTableToRoomPriceTable] @RefrenceNo

							END
						IF (@ProcesstypeId=5)
						BEGIN
						IF NOT EXISTS(SELECT * FROM [reservation].[ApprovalLog] WHERE ProcesstypeId IN (1,2,6,7) AND RefrenceNo=@RefrenceNo AND ApprovalStatus in(0,2,3) AND ApprovalLogId=(SELECT top(1) ApprovalLogId FROM [reservation].[ApprovalLog] WHERE ProcesstypeId IN (1,2,6,7) AND RefrenceNo=@RefrenceNo order by ApprovalLogId desc))
								BEGIN
						...
						END
						 IF (@ProcesstypeId=6)
						BEGIN
							--SET @ApprovalDescripion ='Complementary'
							IF NOT EXISTS(SELECT * FROM [reservation].[ApprovalLog] WHERE ProcesstypeId IN (1,2,5,7) AND RefrenceNo=@RefrenceNo AND ApprovalStatus in(0,2,3) AND ApprovalLogId=(SELECT top(1) ApprovalLogId FROM [reservation].[ApprovalLog] WHERE ProcesstypeId IN (1,2,5,7) AND RefrenceNo=@RefrenceNo order by ApprovalLogId desc))
								BEGIN
									UPDATE reservation.Reservation SET AuthorizedFlag=0 WHERE ReservationID=@RefrenceNo
								END
						END
						 ...
						 IF (@ProcesstypeId=7)
						BEGIN
							--SET @ApprovalDescripion ='Credit Sales'
							IF NOT EXISTS(SELECT * FROM [reservation].[ApprovalLog] WHERE ProcesstypeId IN (1,2,5,6) AND RefrenceNo=@RefrenceNo AND ApprovalStatus in(0,2,3) AND ApprovalLogId=(SELECT top(1) ApprovalLogId FROM [reservation].[ApprovalLog] WHERE ProcesstypeId IN (1,2,5,6) AND RefrenceNo=@RefrenceNo order by ApprovalLogId desc))
								BEGIN
									UPDATE reservation.Reservation SET AuthorizedFlag=0 WHERE ReservationID=@RefrenceNo
								END
						END
			  ...
			  IF(@ApprovalStatus=2)
					BEGIN
						...
						IF (@ProcesstypeId=4)
							BEGIN
								--SET @ApprovalDescripion ='Room/Meal Plan Tariff Changed'
								 UPDATE  Products.RoomDayPrice SET AuthorizedFlag=2,IsActive=0 WHERE PriceID=@RefrenceNo  --InActive
							END
						IF (@ProcesstypeId=5)
							BEGIN
							--SET @ApprovalDescripion ='company advance payment'
								UPDATE reservation.Reservation SET AuthorizedFlag=2 WHERE ReservationID=@RefrenceNo
							END
						IF (@ProcesstypeId=6) -- Complementary
							BEGIN
								UPDATE reservation.Reservation SET AuthorizedFlag=2 WHERE ReservationID=@RefrenceNo
							END
						IF (@ProcesstypeId=7) -- Credit Sales
							BEGIN
								UPDATE reservation.Reservation SET AuthorizedFlag=2 WHERE ReservationID=@RefrenceNo
							END
			END
