...
CREATE PROCEDURE [reservation].[MoveInHOuseGetProductDetails] --45,'12/18/2023','12/21/2023',0,0,0,0
(	
	@SubCategoryID int,
...
BEGIN
		...
		BEGIN
			...
			inner join Products.RoomPrice PRP on PIT.ItemID=PRP.ItemID  -- Added BY Somnath
		...
		--Begin Commented by Arabinda on 2023-09-27 to get the counts from subcategory tables------------------
			...
			Inner join [guest].[GuestCompanyRateContract] GC on IT.ItemID = GC.ItemID
			inner join Products.RoomPrice PRP on IT.ItemID=PRP.ItemID  -- Added BY Somnath
