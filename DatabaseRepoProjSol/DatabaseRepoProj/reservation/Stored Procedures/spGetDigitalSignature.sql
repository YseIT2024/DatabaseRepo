CREATE PROCEDURE [reservation].[spGetDigitalSignature]
(
	@InvoiceNo int
 ...
 --     ,[GuestName] from [reservation].[GuestSignature] where [InvoiceNo]=@InvoiceNo
	...
	Begin
	  ...
	  SELECT 
			...
			(select ManagerSignature from [reservation].[GuestSignature]  where InvoiceNo=@InvoiceNo and IsActive=1) as ManagerSignature,RS.ReservationID
			...
			(select ManagerSignature from [reservation].[GuestSignature]  where InvoiceNo=RI.InvoiceNo and IsActive=1) as ManagerSignature,RS.ReservationID
			FROM [reservation].[Invoice] RI
			INNER JOIN [reservation].[Reservation] RS ON RI.FolioNumber=RS.FolioNumber
			inner join [reservation].[ReservationType] RT on RS.ReservationTypeID = RT.ReservationTypeID
			inner join [reservation].[ReservationMode] RM on RS.ReservationModeID = RM.ReservationModeID
