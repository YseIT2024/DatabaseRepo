CREATE PROC [reservation].[ApproveReservation] --7602,75,
    @ReservationID int,
	@UserID Int,
				...
				UPDATE [reservation].[Reservation]
					...
					WHERE  ReservationID = @ReservationID
					 ...
					 INSERT INTO [reservation].[ReservationStatusLog]
							 ...
							 VALUES (@ReservationID,1,'Reserved',@UserID,GETDATE(),0)
					...
					select @TotalAmountBeforeTax=TotalAmountBeforeTax, @TotalAmountAfterTax=TotalAmountAfterTax from reservation.Reservation where ReservationID=@ReservationID
							...
							--,[CreatedBy])
							--VALUES
							--(@ReservationID,FORMAT(@AUTOID, '000000'),GETDATE(),@UserID)
					...
					SET @Message = 'Reservation confirmed successfully for the Reservation ID : ' + CAST(@ReservationID AS VARCHAR(100));	
