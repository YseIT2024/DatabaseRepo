...
CREATE Proc [reservation].[spGetReservationHistory]--10,1
(	
	@Count int,
	@DrawerID int
)
...
BEGIN
	...
	SELECT [ReservationID],[FolioNumber],[GuestName],[Address],[Adults],[Children],[ExpectedNight],[ExpectedCheckIn],[ExpectedCheckOut]
	,[ActualNight],[ActualCheckIn],[ActualCheckOut],[DateTime],[ReservationStatus],[BillTo]
	FROM
	(
		SELECT 
		ROW_NUMBER() OVER (ORDER BY r.ReservationID DESC) AS RowNumber
		,r.[ReservationID]
