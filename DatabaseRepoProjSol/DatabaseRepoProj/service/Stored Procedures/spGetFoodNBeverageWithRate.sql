...
CREATE PROCEDURE [service].[spGetFoodNBeverageWithRate]
(
	@DrawerID int
)
AS
BEGIN
	...
	WHERE i.LocationID = @LocationID AND i.ServiceTypeID = @FoodNBeverageServiceTypeID
	...
	WHERE i.LocationID = @LocationID AND i.ServiceTypeID = @FoodNBeverageServiceTypeID AND r.IsActive = 1
	ORDER BY i.ItemNumber
END